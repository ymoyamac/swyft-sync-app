...
import { MagicValues } from '../../../enums/MagicValues';
...
export function useAutocomplete({ options = [] }: AutocompleteProps) {
    ...
    useEffect(() => {
        setIsBlank(search === MagicValues.EmptyString);
        filterOptions();
        setActive(0);     
    }, [search]);
    ...
    function filterOptions() {
        ...
        setFilteredOptions(
            [...new Set([
                value.length === MagicValues.One ? value : (!isBlank && value !== options.find(option => option === value)) ? value : MagicValues.EmptyString,
                ...options.filter(option => option.trim().toLocaleLowerCase().includes(value))
            ])]
            .filter(option => option !== MagicValues.EmptyString)
        );
    }
